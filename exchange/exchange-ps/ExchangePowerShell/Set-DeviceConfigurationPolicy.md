...
online version: https://learn.microsoft.com/powershell/module/exchangepowershell/set-deviceconfigurationpolicy
schema: 2.0.0
...
Use the Set-DeviceConfigurationPolicy cmdlets to modify device configuration policies in Basic Mobility and Security. These policies were created by using the New-DeviceConfigurationPolicy cmdlet or on the **Basic Mobility and Security** page with option **If a device doesn't meet the requirements above, then...** \> **Allow access (device enrollment is required)** selected.
...
This example disables the device configuration policy named Engineering Group.
...
The Identity parameter specifies the device configuration policy that you want to modify. You can use any value that uniquely identifies the policy. For example:
