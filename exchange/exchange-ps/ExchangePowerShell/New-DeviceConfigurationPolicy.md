...
Use the New-DeviceConfigurationPolicy cmdlet to create device configuration policies in Basic Mobility and Security. These policies also result from creating policies on the **Basic Mobility and Security** page with option **If a device doesn't meet the requirements above, then...** \> **Block access** selected.
...
This example creates a new device configuration policy named Engineering Group. After you create the policy, use the New-DeviceConfigurationRule cmdlet to create the associated device configuration rule.
...
The policy name (for example, Engineering Group) is used in the name value of the associated rule when you create it (for example, Engineering Group{xxx}).
